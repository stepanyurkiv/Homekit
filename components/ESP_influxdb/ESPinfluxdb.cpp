...
DB_RESPONSE Influxdb::opendb(String db) {
	...
	_http.begin("http://" + _host + ":" + _port + "/query?q=show%20databases"); //HTTP
...
DB_RESPONSE Influxdb::write(String data) {
	...
	_http.begin("http://" + _host + ":" + _port + "/write?db=" + _db); //HTTP	
	_http.addHeader("Content-Type", "text/plain");

	if (!_http.connected()) {
		DEBUG_PRINT("HTTP not connected!");
	}
	...
	if (httpResponseCode == 204) {
		_response = DB_SUCCESS;
		String response = _http.getString();    //Get the response to the request
...
DB_RESPONSE Influxdb::ping() {

	DEBUG_PRINT("Pinging influxdb server " + _host + ":" + _port + " ..." );
	...
	if (httpResponseCode == 200) {
		...
		DEBUG_PRINT("Error on sending ping:");
		DEBUG_PRINT(String(httpResponseCode));
		_response=DB_ERROR;
	}
...
DB_RESPONSE Influxdb::query(String sql) {
	...
	_http.begin("http://" + _host + ":" + _port + url); //HTTP
...
void dbMeasurement::setMeasurement(String m) {
...
String dbMeasurement::postString() {
	...
	return _measurement + _tag + _data;
}
